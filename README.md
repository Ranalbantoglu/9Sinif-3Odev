...
Yukarıdaki örneği kullanarak **kendi ölçümlerinizle** bir tablo oluşturun (Aşağıda yeni bir tablo oluşturun ve yukarıdaki tabloya 3-4 tane kendi ölçümünü ekleyiniz.) Kullandığınız Google Sheet için [buraya](https://docs.google.com/spreadsheets/d/1x2i3ghLmzDxooquA0_9cmxtQcngLz51cy4GC4J_7BaI/edit?usp=sharing) link verin (Google Sheet herkese açık erişimli olsun.) 

a) Denkleminizi **buraya yazınız**. Sizce a, b, ve c'nin fiziksel anlamları ne olabilir? (Yardım: bir tanesi ivme ile ilgili, bir tanesi ilk hız ile, diğeri konum ile ilgili).  Sizce niçin a'nın değeri negatif? x=4,15E-03+1,82t+-1,24t^2 a ivmedir negatif olmasının sebebi hızın giderek azalmasıdır. b hız c konum olabilir.

b) Hava sürtünmesini ihmal edersek, bu harakette ivme sabit midir? sabittir.

c) Video'daki cismin atılış hızı farklı olsaydı (diğer herşey aynı, kütle, yüzey, vb. Hava sürtünmesini ihmal edin), sizce formülde hangi terimler değişecekti? (a, b veya c) b değişir.
...
1) Kendi aldığınız verileri kullanarak **ortalama hız-zaman** grafiğini Google çiziniz. [Buraya](https://docs.google.com/spreadsheets/d/15un1N6ZM04N2v_7XxDz-LFJv6JTwMI-fvAwvepUZwa8/edit?usp=sharing) link veriniz.
...
3) İvme'yi ne buldunuz? Bu ivme değerini kullanarak 1kg'lık cisim ile yüzey arasındaki kinetik sürtünme katsayısını bulunuz. ivmeyi tek bir değer bulamadım. bu yüzden kinetik sürtünme kat sayısını hesaplayamadım.

# Bölüm 3 (Hala Beni Öldürmeyi Düşünmüyorsan)
1) Bu deneyde ölçüm hatalarım var. Zamandaki ölçüm hatam sizce kaç ms (mili saniye) nedir? Konumdaki ölçüm hatam sizce kaç cm'dir? 0,1 cm olabilir.

2) Acaba hiç grafik çizmeden sürtünme katsayısını bulabilir miydim?  (Cevap: Evet). 
Yardım:
* Düzgün ivmelenen bir cisim için alınan yol formülü x = (ilk hız+ son hız)/2*t, burdan cismin ilk hızını bulunuz. 2,6 buldum
* Daha sonra iş-kinetik enerji teorimini kullanarak sürtünme kuvvetini bulun. Sürtünme kuvvetinden, kinetik sürtünme katsayısını bulunuz.W=1/2.m.v^2 0,98 

3) Bu video'dan statik sürtünme katsayısını bulabilir miydim? sanırım hayır bulamayız.
